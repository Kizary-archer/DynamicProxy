...
public class CacheProxy {
    ...
    private Set<Integer> primeNumberCache(Object[] args, MethodProxy proxy, Object obj) throws Throwable {
        Set<Integer> outSet = (Set<Integer>) args[0];
        Set<Integer> tempSet = new HashSet<>(outSet);

        outSet.removeAll(cache.getNumbers());
        if (outSet.size() > 0) {
            cache.getPrimeNumbers().addAll((Set<Integer>) proxy.invokeSuper(obj,args));
            cache.getNumbers().addAll(outSet);
        }
